...
export default function PostulacionesPage() {
  const [postulaciones] = useState<Postulacion[]>(postulacionesAdmin);
  ...
  const postulacionesFiltradas = postulaciones.filter(post => {
    ...
    const matchEstado = filterEstado === "Todas" || post.estado === filterEstado;
    const matchVacante = filterVacante === "Todas" || post.vacanteTitulo === filterVacante;
    const matchExperiencia = filterExperiencia === "Todas" || post.experiencia === filterExperiencia;
  ...
  const currentPostulaciones = postulacionesFiltradas.slice(startIndex, endIndex);
  ...
  const vacantesUnicas = Array.from(new Set(postulaciones.map(p => p.vacanteTitulo)));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {["Nueva", "En revisión", "Entrevista", "Rechazada", "Contratado"].map((estado) => {
      ...
      <div className="bg-white rounded-2xl border border-amber-100 shadow-md p-6">
        ...
        <div className="flex flex-wrap items-center gap-4 mt-4 pt-4 border-t border-amber-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-amber-700">Experiencia:</span>
          </div>
          <select
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-amber-600 rounded-full flex items-center justify-center flex-shrink-0">
                        <span className="text-white font-bold text-xs">
                          {post.candidatoNombre.split(' ').map(n => n[0]).join('')}
                        </span>
                      </div>
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 text-xs text-amber-600">
                          ...
                          <span className="truncate">{post.candidatoEmail}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1 text-sm text-amber-700">
                      ...
                      {new Date(post.fechaPostulacion).toLocaleDateString('es-MX')}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-medium border whitespace-nowrap ${estadoColors[post.estado]}`}>
                      {post.estado}
                    </span>
                  </td>
                  <td className="px-6 py-4">
        ...
        {totalPages > 1 && (
          <div className="border-t border-amber-100 px-6 py-4 flex items-center justify-between">
            <p className="text-sm text-amber-700">
              Mostrando {startIndex + 1} a {Math.min(endIndex, postulacionesFiltradas.length)} de {postulacionesFiltradas.length} resultados
            </p>
            <div className="flex items-center gap-2">
              <button
      ...
      {postulacionesFiltradas.length === 0 && (
        <div className="bg-white rounded-2xl border border-amber-100 p-12 text-center">
          ...
          <h3 className="text-xl font-bold text-amber-950 mb-2">No se encontraron postulaciones</h3>
